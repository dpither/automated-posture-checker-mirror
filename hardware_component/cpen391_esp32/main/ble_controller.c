...
#define TAG "BLE_CONTROLLER"

#define DEFAULT_MTU_SIZE 80
...
#define TEST_DEVICE_NAME            "APC_Setup"
...
{
    ESP_LOGI(TAG, "GAP EVT RECIEVED: 0x%02x\n", event);
    switch (event) {
...
void example_write_event_env(esp_gatt_if_t gatts_if, prepare_type_env_t *prepare_write_env, esp_ble_gatts_cb_param_t *param){
    ...
    if (param->write.need_rsp){
        ...
        uint8_t out_data[DEFAULT_MTU_SIZE] = {0};
        uint16_t out_len;

        // Handle protocol
        if (param->write.value[0] == BLE_START_COMMAND) {
            gpio_set_level(START_STOP_PIN, 1);
            out_data[0] = 0x00;
            out_len = 1;
        } else if (param->write.value[0] == BLE_STOP_COMMAND) {
            gpio_set_level(START_STOP_PIN, 0);
            out_data[0] = 0x00;
            out_len = 1;
        } else if (param->write.value[0] == BLE_SEND_TEST_PHOTO) {
            gpio_set_level(START_STOP_PIN, 1);
            vTaskDelay( 2000 / portTICK_PERIOD_MS );
            gpio_set_level(START_STOP_PIN, 0);
            out_data[0] = 0x00;
            out_len = 1;
        } else if (param->write.value[0] == BLE_WIFI_CONFIGURE) {
            // parse args
            configure_args_t config;
            config.session_id = *((uint32_t *) &param->write.value[1]);
            config.user_id = *((uint32_t *) &param->write.value[5]);
            config.wifi_ssid_length = param->write.value[9];
            int j = 10;
            for (int i = 0; i < config.wifi_ssid_length; i++) {
                config.wifi_ssid[i] = param->write.value[j];
                j++;
            }
            config.wifi_password_length = param->write.value[j];
            j++;
            for (int i = 0; i < config.wifi_password_length; i++) {
                config.wifi_password[i] = param->write.value[j];
                j++;
            }
            g_session_id = config.session_id;
            g_user_id = config.user_id;
            int retVal = wifi_controller_connect(config.wifi_ssid, config.wifi_ssid_length, config.wifi_password, config.wifi_password_length);
            out_data[0] = (uint8_t) retVal;
            out_len = 1;

            char url[128] = {0};
            sprintf(url, "http://3.145.204.90:3000/sessions/%lu/%lu/image", g_user_id, g_session_id);
            ESP_LOGI(TAG, "New url: %s", url);
        } 
        
        esp_ble_gatts_send_indicate(gatts_if, param->write.conn_id, gl_profile_tab.char_handle,
                                out_len, out_data, false);
    }
}
...
{
    ESP_LOGI(TAG, "GATTS EVT RECIEVED: 0x%02x\n", event);
...
void ble_controller_init(void)
{
    ...
    esp_err_t local_mtu_ret = esp_ble_gatt_set_local_mtu(DEFAULT_MTU_SIZE);
    if (local_mtu_ret){
